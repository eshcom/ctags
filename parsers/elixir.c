...
static char *makeScopeName(const char *scope, const char *name, int kindIndex)
{
	...
	if (kindIndex == K_MODULE || kindIndex == K_PROTO ||
		...
		(kindIndex == K_MACRO && strcmp(name, "__using__") != 0))
	{
...
static int makeTag(const char *name, elixirKind kind, bool private,
				   Scope scope, const char *inheritance)
{
	int r = CORK_NIL;
										   ...
										   bool private, int indent)
{
	...
	if (vStringLength(identifier) > 0)
	{
		...
		if (kind == K_MODULE || kind == K_PROTO)
		{
			...
			r = makeTag(search, kind, private, currScope, NULL);
		...
		else if (kind == K_IMPL)
		{
			...
			r = makeTag(ident, kind, private, currScope, NULL);
										   ...
										   elixirKind kind, bool private)
{
	...
	if (vStringLength(identifier) > 0)
	{
		...
		makeTag(ident, kind, private, currScope, module);
		FREE_SCOPE(currScope);
		free(module);
	}
	vStringDelete(identifier);
										   ...
										   const char *name)
{
	...
	if (kind == K_ATTRIBUTE)
		makeTag(name, kind, private, currScope, NULL);
	else if (currScope.name) // K_STRUCT
		makeTag(currScope.name, kind, private, currScope, NULL);
...
/*
 * K_OTHER is not needed for ctag-files (it is only needed for display in geany)
										  ...
										  bool is_code_block, int indent)
{
	...
	Scope currScope = getCurrentScope();
	int r = makeTag(name, K_OTHER, false, currScope, NULL);
	FREE_SCOPE(currScope);
...
static const unsigned char *parseSpecialTag(const unsigned char *cp, const char *name)
{
	...
	if (vStringLength(identifier) > 0)
	{
		...
		makeTag(name, K_SPECIAL, true, currScope, vStringValue(vFullName));
		vStringDelete(vFullName);
		FREE_SCOPE(currScope);
	}
	vStringDelete(identifier);
